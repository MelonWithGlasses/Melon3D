...
1. **`dump_scene.cpp`** links `melon3d`, simulates the showcase scene
   (a box pyramid struck by a projectile, then a rain of mixed shapes),
   and writes every frame's body transforms to a binary file `scene.bin`.
2. **`render.py`** reads `scene.bin`, renders each frame with a small
   ...
   a floor grid) and writes `melon3d.gif`.
